  ...
  return (
    <main className="min-h-screen bg-surface px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        <Link className="mb-5 inline-flex items-center gap-2 text-sm text-slate-600" to="/profile">
          <ArrowLeft size={16} />
          Back to profile
        </Link>
        ...
        <header className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">Admin-only user management.</p>
          </div>
        ...
        <section className="rounded-lg border border-line bg-white">
          {adminPeopleQuery.isLoading ? (
            <div className="p-6 text-sm text-slate-500">Loading people...</div>
          ) : adminPeopleQuery.isError ? (
            <div className="p-6 text-sm text-red-700">Could not load people.</div>
          ) : (
                ...
                <tbody className="divide-y divide-line">
                  {(adminPeopleQuery.data ?? []).map((person) => (
                    <tr key={person.id} className="h-16">
                      <td className="truncate px-4 py-3" title={person.name}>
                        {person.name}
                      </td>
                      <td className="truncate px-4 py-3" title={person.email}>
                        {person.email}
                      </td>
                      <td className="truncate px-4 py-3" title={person.contactNumber ?? "-"}>
                        {person.contactNumber ?? "-"}
                      </td>
                      <td className="truncate px-4 py-3" title={person.department?.name ?? "No department"}>
